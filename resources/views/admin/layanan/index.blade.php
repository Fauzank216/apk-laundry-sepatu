    ...
    <div class="bg-white border border-gray-300 overflow-x-auto">
        <table class="text-xs md:text-base rounded-sm min-w-[700px] w-full text-center">
            ...
            <tbody>
                ...
                <tr>
                    <td class="p-2 border-b border-gray-200">{{$service->id}}</td>
                    <td class="p-2 border-b border-gray-200">{{$service->name}}</td>
                    <td class="p-2 border-b border-gray-200">Rp. {{Number::format($service->price, locale:'idn')}}</td>
                    <td class="p-2 border-b border-gray-200">null</td>
                    <td class="p-2 border-b border-gray-200">
                        <textarea class="w-full max-h-[50px] border border-gray-200 rounded-sm p-2" name="#" id=""
                            readonly>{{$service->description}}</textarea>
                    </td>
                    <td class="p-2 border-b border-gray-200">
                        <button
                            onclick="showModal('confirm-delete-layanan', 'http://127.0.0.1:8000/admin/services/{{$service->id}}')"
                        ...
                        <button
                            onclick="showModal('modal-layanan', 'http://127.0.0.1:8000/admin/services/{{$service->id}}', this)"
                            data-id="{{$service->id}}" data-name="{{$service->name}}" data-price="{{$service->price}}"
                            data-description="{{$service->description}}" class="cursor-pointer text-yellow-500">
                            <svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                class="size-4 md:size-6">
                                <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div class="w-full max-w-[560px] overflow-hidden rounded-md border border-gray-300 bg-white shadow-xl">
        ...
        <div class="px-3 py-2 md:px-6 md:py-5">
            <form action="{{route('services.store')}}" method="POST">
                ...
                <div class="mb-4">
                    <label class="mb-2 block text-xs md:text-base font-medium text-slate-700" for="price">
                        Harga <span class="text-red-500">*</span>
                    </label>

                    <input id="price" name="price" type="text" placeholder="Masukkan harga layanan" autocomplete="off"
                        class="form-control w-full rounded-md border border-slate-300 px-4 py-2.5 text-xs md:text-base text-slate-700 placeholder-slate-400 outline-none transition focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100">
                </div>

                <div class="mb-4">
                    <label class="mb-2 block text-xs md:text-base font-medium text-slate-700" for="description">
                        Deskripsi <span class="text-red-500">*</span>
                    </label>

                    <textarea name="description"
                        class="form-control text-xs md:text-base w-full min-h-[40px] border border-gray-200 rounded-sm p-2"
                        placeholder="Contoh : Lorem ipsum dolor sit amet consectetur, adipisicing elit. Repudiandae, voluptas."></textarea>
                </div>

                <div class="mb-5">
                    <label class="mb-2 block text-xs md:text-base font-medium text-slate-700" for="thumbnail">
                        Thumbnail <span class="text-red-500">*</span>
                    </label>

                    <input name="thumbnail" class="text-xs md:text-base w-full border border-gray-200 rounded-sm p-2"
                        type="file">
                </div>
